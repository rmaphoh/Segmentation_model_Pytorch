...
class RandomElastic(object):
    ...
    def __call__(self, img, target = None):
        ...
        if target is not None:
            return self.RandomElasticCV2(np.array(img), self.alpha, self.sigma), self.RandomElasticCV2(np.array(target), self.alpha, self.sigma)
        return self.RandomElasticCV2(np.array(img), self.alpha, self.sigma)
